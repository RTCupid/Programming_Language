...
Для компиляции таких операторов, как ввод, вывод и завершение программы потребовалось написать библиотеку с их реализацией на ассемблере NASM [[10]](/common/lib/stdlib.s). Библиотека подключается при помощи директивы "%include", и в местах использования этих операторов в основной код вставляется конструкция `call "имя функции для обработки оператора"`.
