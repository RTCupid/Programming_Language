...
Бэкенд для процессора x86-64 поддерживает полный функционал языка, в то время как бэкенд для эмулятора процессора не поддерживает синтаксис функций, поэтому для тестирования производительности генерируемого кода принято решение использовать синтаксис цикла с вычислениями. Для сравнения производительности кода, полученного при компиляции для эмулятора процессора, и при компиляции для процессора `x86-64` (см. Рис. 1), использовался следующий код на "языке Самурая" [[10]](/programs_in_my_language):
...
По результатам измерений (см. Табл. 1) видно, что код на процессоре `х86-64` выполнялся в `530` раз быстрее, чем на эмуляторе процессора, несмотря на то, что производились одинаковые действия.  
