...
Создан новый язык программирования - "Язык самурая". Он имеет С-подобный синтаксис и содержит операции, необходимые для написания программы для решения квадратных уравнений и программы для вычисления факториала числа.
...
Для выполнения программ на "языке Самурая" - созданном языке программирования, выбраны процессор с архитектурой x86-64 (см. Рис. 1) и эмулятор процессора CPU (см. проект [Processor](https://github.com/RTCupid/Proccessor)).
...
В промежуточном представлении код представлялся полной скобочной записью в префиксном формате. В миддленде из этого представления восстанавливалось абстрактное синтаксическое дерево, для которого проводились такие оптимизации, как свёртка констант и удаление неиспользуемого кода (нейтральных выражений). 

В бэкенде также восстанавливалось абстрактное синтаксическое дерево из текстового файла, производился обход дерева, в процессе которого генерировался код на ассемблере. Этот код компилировался дальше при помощи nasm.
